...
def set_task_tags(task_id, add=(), drop=(), soft=False):
    ...
    for attempt in range(1, TAG_SETTLE_TRIES + 1):
        ...
        if lost and attempt == TAG_SETTLE_TRIES:
            die(f"{task_id}: запись тегов потеряла чужие метки {sorted(lost)}.\n"
                "  Скорее всего их добавил другой агент между чтением и записью, и "
                "это потеря следа «кто взял задачу».\n"
                "  Но отсюда это неотличимо от отставшей очереди трекера: повтори "
                "команду — метки вернулись значит очередь, нет значит потеря.")
    ...
    die(f"{task_id}: правка тегов не применилась за {TAG_SETTLE_TRIES} "
        ...
        "  Причин две, и отсюда они неразличимы: очередь синхронизации трекера "
        "ещё не применила запись, либо сервер принял её и не применил.\n"
        "  Различает повтор: та же команда прошла — была очередь; упала снова — "
        "смотри задачу в трекере.")
...
# --------------------------------------------------------------------- счётчики лимита
#
# Сервер называет остаток лимита САМ — но только в успешном ответе. На каждом `200`
# лежат шесть заголовков: `X-RateLimit-{Limit,Remaining,Reset}-short` (100 запросов,
# окно 60 с) и то же с `-long` (1000 запросов, окно 3600 с). `Reset` — СКОЛЬКО СЕКУНД
# осталось до сброса окна, и он тикает: замер 19.09 — 17 с, через 6 секунд 11 с.
# На самом `429` заголовков нет вовсе, поэтому учёт строится по успехам ДО отказа.
#
# Счётчик свой у каждой пары (МЕТОД, шаблон маршрута) — это замерено, а не выведено
# из документации, которой нет (числа — в references/api.md).
RATE_RESERVE = 2       # запросов бакета, которые не тратим: остаток — оценка, а не истина
RATE_WAIT_CAP = 65     # секунды: потолок ОДНОЙ упреждающей паузы (минутное окно + запас)
# `Reset` приходит ЦЕЛЫМИ секундами, то есть уже округлён — и, судя по наблюдаемым
# значениям, вниз. Проснуться на долю секунды раньше сброса значит получить ровно
# тот `429`, которого пауза и должна была избежать, поэтому к паузе добавляется
# запас чуть больше секунды. Цена ошибки несимметрична: лишняя секунда сна дешевле
# отказа команды.
RATE_WAIT_PAD = 1.5

# Сегмент пути, который на самом деле идентификатор: `T-<uuid>`, `KS-P-<uuid>-TODO`,
# числовой id. Короткий буквенный префикс до дефиса отличает их от имён ручек
# (`kanban-status`, `checklist-item` — префикс длиннее пяти букв).
_ID_SEGMENT = re.compile(r"^\d+$|^[A-Za-z]{1,5}-[0-9A-Za-z-]{8,}$")

# Что известно про каждый бакет, по последнему ответу сервера. Состояние живёт в
# ПРОЦЕССЕ и умирает вместе с командой — см. rate_pause про то, почему этого хватает
# и почему числа всё равно остаются оценкой.
_RATE = {}
_RATE_WARNED = set()


def rate_bucket(method, path):
    """Ключ счётчика: метод + шаблон маршрута, без query.

    Две РАЗНЫЕ задачи по id идут одним счётчиком (замер: 92 → 91 → 90 подряд),
    `GET /task` — другой (в ту же секунду 98), `POST /task` — третий (99 при
    минутном окне, только что открытом). Поэтому id из пути вычищается, а метод
    в ключ входит.

    Идентификатором считается сегмент вида `T-<uuid>` — короткий префикс и
    длинный хвост; выдуманный короткий `T-1` под правило не подходит, живые id
    трекера подходят все. Ошибки тут несимметричны, и опасная — именно эта:
    НЕ распознанный id дробит один серверный счётчик на много клиентских, каждый
    выглядит почти полным, и упреждение не сработает вовсе. Обратная ошибка
    (принять имя ручки за id) всего лишь склеит разные счётчики в один — оценка
    станет пессимистичнее. Поэтому форма ключа закреплена проверкой
    (`tests/test_limits.py`), а не «и так видно».
    """
    parts = ["{id}" if _ID_SEGMENT.match(seg) else seg for seg in path.split("/")]
    return f"{method} {'/'.join(parts)}"


def rate_note(bucket, headers):
    """Запомнить, что сервер сказал про остаток. Нет заголовков — состояние не трогаем.

    Отсутствие заголовков — это не «лимита нет», а «в этом ответе про него не
    сказано»: ровно так выглядит `429`. Затирать им прежнее знание нельзя.
    """
    if not headers:
        return None
    seen = {}
    for window in ("short", "long"):
        try:
            limit = int(headers.get(f"X-RateLimit-Limit-{window}"))
            left = int(headers.get(f"X-RateLimit-Remaining-{window}"))
            reset = float(headers.get(f"X-RateLimit-Reset-{window}"))
        except (TypeError, ValueError):
            continue
        seen[window] = {"limit": limit, "left": left, "reset_at": time.time() + reset}
    if not seen:
        return None
    _RATE.setdefault(bucket, {}).update(seen)
    return _RATE[bucket]


def rate_spend(bucket):
    """Свой запрос списать СРАЗУ, не дожидаясь ответа.

    Ответ может не принести заголовков (`429`, обрыв сети) — и тогда без этого
    списания остаток остался бы прежним, то есть скилл считал бы, что запас цел,
    ровно в тот момент, когда он кончился.
    """
    st = _RATE.get(bucket)
    if not st:
        return
    now = time.time()
    for window, data in st.items():
        if now < data["reset_at"]:
            data["left"] = max(0, data["left"] - 1)


def rate_known(bucket, window, now=None):
    """Остаток бакета в окне или None, если окно уже сброшено (знание протухло)."""
    data = (_RATE.get(bucket) or {}).get(window)
    if not data:
        return None
    if (time.time() if now is None else now) >= data["reset_at"]:
        return None
    return data


def rate_pause(bucket, sleep=None, out=None):
    """Притормозить ПЕРЕД запросом, если минутное окно ручки на исходе.

    Цель — не доводить до `429`, а не лучше из него выходить: в самом отказе
    сервер не говорит ничего, ждать там по-прежнему приходится вслепую
    (THROTTLE_RETRIES, Accepted — см. references/api.md).

    Три честных ограничения, которые здесь не замаскированы:

    * **Остаток — оценка, а не истина.** Известен он по СВОЕМУ последнему ответу
      этой ручки, а счётчик общий на токен: параллельные сессии (их тут бывает
      несколько) тратят тот же лимит и в наших числах не видны. Поэтому
      напечатанный остаток — потолок: на сервере его не больше, а может быть и
      меньше. Отсюда `RATE_RESERVE`: тормозим, не досчитав до нуля.
    * **Состояние живёт в процессе.** Одна команда агента — один процесс, и
      знание с ней умирает; помогает это там, где запросы идут очередью внутри
      одной команды (выборка страницами, живой набор тестов ~300 запросов).
      Разделяемого файла намеренно нет: он добавил бы гонку между сессиями и
      протухание, а оценкой остался бы всё равно.
    * **Часовое окно пережидать нельзя.** До его сброса бывает 50 минут; спать
      столько бессмысленно, поэтому про него только предупреждение.

    Выключается на запуск: `SINGULARITY_NO_RATE_WAIT=1` (нужен, чтобы замерить
    поведение БЕЗ упреждения — контрольный красный прогон).
    """
    out = out or sys.stderr
    if os.environ.get("SINGULARITY_NO_RATE_WAIT"):
        return 0.0
    now = time.time()
    long_left = rate_known(bucket, "long", now)
    if long_left and long_left["left"] <= RATE_RESERVE and bucket not in _RATE_WARNED:
        _RATE_WARNED.add(bucket)
        print(f"⚠ {bucket}: часовой счётчик почти исчерпан ({long_left['left']} из "
              f"{long_left['limit']}), сброс через {(long_left['reset_at'] - now) / 60:.0f} мин. "
              "Столько не жду — дальше возможен 429.", file=out)
    short = rate_known(bucket, "short", now)
    if not short or short["left"] > RATE_RESERVE:
        return 0.0
    wait = min(short["reset_at"] - now + RATE_WAIT_PAD, RATE_WAIT_CAP)
    if wait <= 0:
        return 0.0
    print(f"⏳ {bucket}: осталось {short['left']} из {short['limit']} в минутном окне — "
          f"жду {wait:.1f} с до сброса, чтобы не упереться в 429.", file=out)
    # `time.sleep` берётся в момент вызова, а не в сигнатуре: значение по умолчанию
    # связалось бы при импорте, и подменённые в проверках часы (tests/test_limits.py)
    # молча не работали бы — проверка спала бы по-настоящему либо врала.
    (sleep or time.sleep)(wait)
    # После сна прежние числа — прошлое окно. Новые принесёт первый же ответ;
    # до него остаток считается неизвестным, а не «полным».
    (_RATE.get(bucket) or {}).pop("short", None)
    return wait


def rate_report():
    """Остаток по ручкам, которые трогал ЭТОТ запуск: [(бакет, строка), …]."""
    lines, now = [], time.time()
    for bucket in sorted(_RATE):
        parts = []
        for window, human in (("short", "минута"), ("long", "час")):
            data = rate_known(bucket, window, now)
            if not data:
                parts.append(f"{human} — окно сброшено")
                continue
            left_s = data["reset_at"] - now
            when = f"{left_s:.0f} с" if left_s < 120 else f"{left_s / 60:.0f} мин"
            parts.append(f"{human} {data['left']}/{data['limit']} (сброс через {when})")
        lines.append((bucket, "   ".join(parts)))
    return lines
...
def request(method, path, query=None, body=None, soft=False):
    ...
    bucket = rate_bucket(method, path)
    attempt, waited = 0, 0.0
    while True:
        attempt += 1
        # Упреждающая пауза идёт ДО отправки и только когда счётчик этой ручки на
        # исходе: на ровном месте она не срабатывает — обычная команда тратит
        # единицы запросов на бакет, а порог стоит в двух от сотни.
        #
        # Только на ПЕРВОЙ попытке и мимо RETRY_WAIT_BUDGET — это два разных
        # механизма, и смешивать их нельзя. После `429` сервер не сообщает ничего
        # (заголовков в отказе нет), остаток мы к тому моменту только списываем
        # вслепую — и пауза на каждой из четырёх попыток превратила бы один запрос
        # в четыре минуты сна вместо внятного отказа за 9 с.
        if attempt == 1:
            rate_pause(bucket)
        rate_spend(bucket)
        try:
            with urllib.request.urlopen(req, timeout=NET_TIMEOUT) as resp:
                rate_note(bucket, resp.headers)
                raw = resp.read().decode()
                return json.loads(raw) if raw.strip() else {}
        except urllib.error.HTTPError as e:
            # На 429 заголовков нет — rate_note такой ответ не учитывает и прежнее
            # знание не затирает; на прочих ошибках они бывают и годятся.
            rate_note(bucket, e.headers)
...
def assert_task_allowed(task_id, cfg=None, fallback=False):
    ...
    `fallback=True` — прочитать задачу в обход троттлинга ручки `/task/{id}`
    (`get_task_data`). Включается ТОЛЬКО у команд, которые дальше ничего не
    пишут, и это не вкусовщина, а разбор последствий:

    * `show` при обходе работает вместо того, чтобы отказать. Ровно этого не
      хватало в инциденте 18.09: после серии прогонов `show` отвечал `429`
      сорок две минуты, а выборка по проекту всё это время работала.
    * Пишущей команде (`done`, `block`, `report`, `move`, `set`…) обход
      сделал бы ХУЖЕ. Сейчас она упирается в `429` здесь — до первой записи,
      и это чистый отказ: в трекере не изменилось ничего. С обходом она
      прошла бы проверку и умерла дальше, на подтверждении записи, которое
      читает ту же зажатую ручку, — но уже ПОСЛЕ `PATCH` заметки. Получилась
      бы наполовину закрытая задача (отчёт в карточке есть, колонка не
      двинулась), а повторять такой `done` надо с `--no-report`, иначе отчёт
      задвоится. Отказ раньше первой записи дешевле отказа посередине.
    """
    task = get_task_data(task_id) if fallback \
        else request("GET", f"/task/{task_id}")
...
def get_task_data(task_id):
    """Задача по id, устойчиво к троттлингу ИМЕННО ручки `GET /task/{id}`.

    Лимит трекера считается ПО РУЧКЕ, и сервер сам это показывает: на каждом
    успешном ответе лежат `X-RateLimit-*-short` (100 запросов / 60 с) и
    `-long` (1000 / ~25 мин), у каждой ручки — свой счётчик. Замер 19.09:
    ровно 100 ответов `200` на `GET /task/{id}`, 101-й — `429`, и в ту же
    секунду `GET /task?projectId=…` отвечает `200` со своим счётчиком 99, а
    `GET /project` — `200`. Отсюда обход: ту же задачу отдаёт выборка по
    проекту, и это НЕ бедный суррогат — сверка на живой задаче дала 39 ключей
    против 39, ни одного расхождения в значениях, а правка `priority` стала
    видна обеим ручкам одновременно (один и тот же `modificatedDate`).

    Цена обхода честная: `GET /task/{id}` — 0,15 с и 5,5 КБ, выборка по
    проекту в 62 задачи — 0,65 с и 743 КБ (на проекте в 1150 задач это уже
    ~6 запросов и десятки мегабайт). Поэтому обход — запасной путь, а не
    основной: сначала всегда прямое чтение.

    Три свойства, за которые здесь отвечает именно такой порядок:

    * **Отказ остаётся отказом.** Не нашли ни там, ни там — последним
      действием идёт ЖЁСТКИЙ `GET /task/{id}`, и наружу выходит настоящая
      ошибка сервера (`429`, `404`, `401`) со своим текстом. `soft`-режима у
      этой функции намеренно нет: он превратил бы «не смогли прочитать» в
      `None`, а вызывающий код читает `(… or {}).get("tags")` как «тегов
      нет» — и молча теряет чужие метки вместо отказа.
    * **Область не расширяется.** Выборка идёт только по ПРИВЯЗАННОМУ
      проекту, поэтому запасным путём не может прийти задача, которую
      ограничение области (AGENTS.md §3) не пропустило бы: чужая задача в
      выборку не попадает, и наружу выходит ошибка, а не карточка.
    * **Обход не годится там, где дальше идёт запись.** Подробности —
      в `assert_task_allowed`.
    """
    task = request("GET", f"/task/{task_id}", soft=True)
    if task:
        return task
    # check_scope=False намеренно: область проверяет вызывающий (assert_task_allowed),
    # а здесь лишняя проверка стоила бы ещё одного обхода дерева проектов — по той
    # самой сети, которая уже отказывает.
    cfg, _ = load_config(required=False, check_scope=False)
    pid = (cfg or {}).get("projectId")
    if pid:
        match = next((t for t in fetch_tasks(pid, include_archived=True)
                      if t.get("id") == task_id), None)
        if match:
            return match
    return request("GET", f"/task/{task_id}")
...
# суток по UTC. Полдень UTC держит тот же календарный день при сдвиге от -11:59
...
#
# Это защита на ЗАПИСИ и только на наших датах. Чужие (приложение и генератор
# серий пишут локальной полночью) защищены на ЧТЕНИИ — `local_date`, см. её
# docstring: срез строки там давал предыдущий день.
...
def local_date(raw):
    """Датное поле задачи как КАЛЕНДАРНАЯ ДАТА в зоне этой машины (ГГГГ-ММ-ДД).

    ⚠️ Срезать строку (`raw[:10]`) нельзя — это дата по UTC, а не та, которую
    человек видит в карточке. Замер по живой базе приложения (19.09.2026,
    2088 задач с `start`): 970 из них стоят ровно на `21:00:00Z` — это полночь
    по Москве (+03), то есть приложение хранит выбранный человеком ДЕНЬ как
    его локальную полночь в UTC. Срез такой строки даёт ПРЕДЫДУЩИЙ день.

    Чем это обошлось (T-d4d2eac7). Экземпляр повторяющейся серии несёт дату в
    том же `start`: у карточки `…-20260921` там `2026-09-20T21:00:00.000Z`.
    Срез давал `2026-09-20`, поэтому доска писала «начало 2026-09-20» на задаче
    21 сентября, а 20-го сравнение `start > today` становилось ложным и `next`
    выдавал её за сутки до срока — ровно «задача из будущего». Замер по той же
    базе: у 914 экземпляров серии из 967 суффикс id (`-ГГГГММДД`, день
    генерации) совпадает с ЛОКАЛЬНОЙ датой `start` и лишь у 4 — с UTC-датой.
    То есть локальная дата и есть то, что имеет в виду и человек, и приложение.

    Мы сами пишем дату полднем UTC (`DATE_ONLY_TIME`) именно чтобы день не
    съезжал; эта функция — та же защита на ЧТЕНИИ, в том числе для дат, которые
    писали не мы.

    Непонятную строку не выдумываем: возвращаем прежний срез — хуже, чем было,
    от этого не станет. Пусто -> None.
    """
    moment = date_instant(raw)
    if moment is None:
        return (raw or "").strip()[:10] or None
...
def start_after_deadline(task):
    ...
    if s and d and s > d:
        # Даты — локальные (`local_date`): текст предупреждения предлагает
        # готовую команду `set --start … --deadline …`, и UTC-срез подставил бы
        # в неё день, которого человек в карточке не видел.
        return local_date(task.get("start")), local_date(task.get("deadline"))
    return None
...
def brief(t, extra=""):
    # Дедлайн — тот же класс, что и `start`: приложение пишет его локальной
    # полночью (замер: `21:00:00Z` и `20:59:59Z` в живой базе), и UTC-срез
    # показывал бы предыдущий день. Дата берётся локальная — см. `local_date`.
    dl = f" дедлайн={local_date(t['deadline'])}" if t.get("deadline") else ""
...
def task_json(task, role=None, column_name=None, tags=(), group_title=None,
    ...
    d = {
        ...
        # Ровно та дата, что стоит в пометке `[начало ГГГГ-ММ-ДД]` и по которой
        # решает `next` (локальная, не UTC-срез) — иначе машинный вывод спорил
        # бы с человеческим и с поведением очереди.
        "start": local_date(task.get("start")),
...
def cmd_doctor(args):
    ...
    try:
        ...
        # Именно finally: когда проба записи упала на `429`, остаток по ручкам —
        # первое, что объясняет причину. Потерять его вместе с выходом нельзя.
        doctor_limits()


def doctor_limits():
    """Остаток лимита по ручкам — из ответов, которые doctor и так сделал.

    Ни одного запроса ради самого учёта: числа приходят заголовками на каждом
    успешном ответе. Поэтому и список ручек тут ровно тот, куда doctor сходил.

    Оговорка в шапке не украшение: счётчик общий на токен, и параллельные
    сессии тратят его молча. Наше число — потолок (на сервере не больше,
    может быть меньше), а не остаток «на самом деле».
    """
    lines = rate_report()
    if not lines:
        return
    print("\nОстаток лимита по ручкам (100 запросов в минуту и 1000 в час НА РУЧКУ).")
    print("  Числа — из ответов этого запуска; счётчик общий на токен, параллельные")
    print("  сессии тратят его тоже, поэтому это потолок, а не точный остаток.")
    for bucket, text in lines:
        print(f"  {bucket:<28} {text}")
...
def starts_later(task, today=None):
    ...
    Дата берётся ЛОКАЛЬНАЯ (`local_date`), а не срез UTC-строки: приложение
    хранит выбранный день как локальную полночь, и срез сдвигал сравнение на
    сутки назад — задача выдавалась за день до своего срока. Подробности замера
    — в `local_date`.
    """
    start = local_date(task.get("start"))
...
def cmd_show(args):
    cfg, _ = load_config(required=False)
    # fallback=True — `show` ничего не пишет, и читать карточку он обязан даже
    # когда ручка `/task/{id}` зажата лимитом (см. assert_task_allowed).
    t = assert_task_allowed(args.id, cfg, fallback=True)
...
def closed_mark(task):
    """Чем датировать «эту задачу уже закрывали»: даты закрытия у задачи НЕТ.

    Замер по живой доске (19.09, 60 карточек): у всех 55 закрытых `complete` = 0,
    `completeLast` = None — момент закрытия трекер не хранит вовсе. Следа два, и
    оба косвенные: `journalDate` — когда приложение унесло карточку в дневник
    (медиана задержки после закрытия 23 минуты, но бывает и 284 суток, см.
    references/api.md), `modificatedDate` — последняя правка (epoch мс строкой),
    у закрытой задачи это обычно её же `done --report`.

    Поэтому фраза называет ИСТОЧНИК даты, а не выдаёт её за дату закрытия:
    ошибиться в такой мелочи здесь значит соврать в единственном факте, ради
    которого предупреждение и читают.
    """
    if task.get("journalDate"):
        return f"в дневнике с {local_date(task['journalDate'])}"
    try:
        day = datetime.datetime.fromtimestamp(
            int(task.get("modificatedDate")) / 1000).date().isoformat()
    except (TypeError, ValueError):
        return "когда закрыта — трекер не говорит"
    return f"последняя правка {day}"


def dup_twins(tasks, title):
    """Двойники по заголовку среди задач проекта: (открытые, закрытые).

    На вход идёт ДОСКА ЦЕЛИКОМ (board_tasks), включая унесённое в дневник:
    открытые здесь — это ровно прежний open_tasks (не закрыта и не в дневнике),
    а закрытые раньше не искались вовсе.

    Закрытая — это `checked=1` ИЛИ `journalDate`, одного поля мало. Приложение
    уносит закрытое в дневник не сразу (на этой доске 40 закрытых карточек с
    `journalDate` и 15 без него), а архивируя проект целиком, уносит и
    невыполненное — тогда `journalDate` есть, а `checked` нет.

    Повторяющаяся серия закрытым двойником НЕ считается — ни шаблон
    (`recurrence`), ни порождённые им экземпляры (`recurrenceGeneratorId`).
    Экземпляр совпадает по заголовку с каждым предыдущим ПО УСТРОЙСТВУ серии:
    на этой доске все три группы одинаковых заголовков — серии (6, 2 и 2
    карточки), то есть предупреждение срабатывало бы только на них и всегда. А
    предупреждение, которое срабатывает всегда, перестают читать — вместе с тем
    единственным случаем, ради которого оно написано.

    Открытый экземпляр серии двойником остаётся: его не надо заводить заново,
    он уже стоит на доске, и прежний отказ здесь — правильный ответ.
    """
    open_twins, done_twins = [], []
    for t in tasks:
        if not same_title(t.get("title"), title):
            continue
        if int(t.get("checked") or 0) == 1 or t.get("journalDate"):
            if t.get("recurrence") is None and not t.get("recurrenceGeneratorId"):
                done_twins.append(t)
        else:
            open_twins.append(t)
    return open_twins, done_twins
...
def cmd_add(args):
    ...
    if not args.dup_ok:
        # Выборка — доска ЦЕЛИКОМ, включая дневник, и одна на обе проверки.
        # Раньше здесь стоял open_tasks(), и уже сделанная работа была невидима:
        # агент заводил её заново, а отказ так и писал «уже открыта» (T-cf565316).
        # `init` свою защиту от дублей всегда строил на board_tasks — `add`
        # оставался единственным местом, где выборка обрывалась на открытых.
        # Цена та же: обе выборки — один GET /task (замер 19.09: add — 8 запросов
        # до правки и 8 после, 60 задач проекта против окна в 200).
        twins, done_twins = dup_twins(board_tasks(cfg["projectId"]), args.title)
        if twins:
            cmap = column_map(cfg["projectId"])
        ...
        if done_twins:
            # Предупреждение, а не отказ: повторяемая работа законна (ротация
            # журнала, чистка веток — такие карточки на этой доске есть), и
            # отказ на неё научил бы жать --dup-ok не глядя, то есть выключать
            # заодно и защиту от открытого двойника. Печатается ДО создания:
            # упади POST — факт «это уже делали» всё равно уже сказан.
            lines = "\n".join(f"    {t['id']}  {closed_mark(t)}\n"
                              f"      {task_link(t['id'])}" for t in done_twins)
            print(f"  ⚠ такое уже делали и закрыли — {len(done_twins)} шт. "
                  f"(задача всё равно создаётся):\n{lines}\n"
                  "    повторяемая работа — так и надо; та же самая — прервись и "
                  "прочитай,\n    чем она кончилась: sing.py show <id>  ·  "
                  "заглушить проверку целиком: --dup-ok")
...
def rename_task(task_id, new_title, task=None):
    ...
    for attempt in range(1, TAG_SETTLE_TRIES + 1):
        ...
        die(f"{task_id}: заголовок не применился за {TAG_SETTLE_TRIES} "
            ...
            "  Причин две, и отсюда они неразличимы: очередь синхронизации "
            "трекера ещё не применила запись, либо сервер принял её и не "
            "применил.\n"
            "  Различает повтор: та же команда прошла — была очередь; упала "
            "снова — смотри задачу в трекере.")
...
def set_task_fields(task_id, fields, task=None, watched=SET_WATCHED):
    ...
    for attempt in range(1, TAG_SETTLE_TRIES + 1):
        ...
        die(f"{task_id}: правка не применилась за {TAG_SETTLE_TRIES} "
            f"перечитываний ({TAG_SETTLE_PAUSE * (TAG_SETTLE_TRIES - 1):.0f} с):\n"
            f"{lines}\n"
            "  Сервер ответил успехом, но поле не изменилось. Причин две, и "
            "отсюда они неразличимы: очередь синхронизации трекера ещё не "
            "применила запись, либо сервер принял её и не применил.\n"
            "  Различает повтор: та же команда прошла — была очередь; упала "
            "снова — смотри задачу в трекере.")
...
def build_parser():
    ...
    sp.add_argument("--dup-ok", action="store_true",
                    help="не проверять двойников: ни отказа на открытого, "
                         "ни предупреждения на уже закрытого")
